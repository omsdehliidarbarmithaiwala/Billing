...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function generatePdfPromise() {
    ...
    // Make visible in fixed position off user view for exact bounding measurement
  ...
  function triggerShopBackupAuto() {
    ...
    setTimeout(function () {
      ...
      showNotification('🛡️ Shop record copy backed up to +91 9820260299!');
    }, 1200);
  }

  // 1-TAP WHATSAPP DISPATCH TO CUSTOMER + AUTOMATIC SHOP BACKUP
  document.getElementById('btn-send-whatsapp').addEventListener('click', async function () {
    ...
    try {
      ...
      // Mobile Web Share API with attached PDF file
      if (pdfResult && navigator.canShare && navigator.share) {
        ...
        if (navigator.canShare({ files: [file] })) {
          ...
          showNotification('✅ Bill shared with PDF attached! Auto-backup to shop triggered.');
      ...
      // Desktop / Standard Fallback: Auto-download PDF & open customer WhatsApp chat
      ...
      // Trigger automatic shop backup
      triggerShopBackupAuto();
      showNotification('✅ Customer WhatsApp opened & PDF downloaded. Auto-backup to 9820260299 sent!');
    } catch (err) {
  ...
  // RE-SEND SHOP RECORD BACKUP (+91 9820260299)
  document.getElementById('btn-send-backup').addEventListener('click', async function () {
    ...
    try {
      ...
      window.open(waUrl, '_blank');
      showNotification('🛡️ Shop record opened for +91 9820260299');
    } catch (e) {
      var waUrl = 'https://wa.me/' + SHOP_BACKUP_PHONE + '?text=' + encodeURIComponent(msg);
      window.open(waUrl, '_blank');
    } finally {
