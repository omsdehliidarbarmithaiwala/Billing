...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function switchMobileView(view) {
    if (view === 'cart') {
      if (tabCatalogueBtn) tabCatalogueBtn.classList.remove('active');
      if (tabCartBtn) tabCartBtn.classList.add('active');
      if (catalogueCard) catalogueCard.classList.add('mobile-hidden');
      if (billCard) billCard.classList.remove('mobile-hidden');
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } else {
      if (tabCatalogueBtn) tabCatalogueBtn.classList.add('active');
      if (tabCartBtn) tabCartBtn.classList.remove('active');
      if (catalogueCard) catalogueCard.classList.remove('mobile-hidden');
      if (billCard) billCard.classList.add('mobile-hidden');
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }
  }
  ...
  function renderCatalogue() {
    ...
    var html = filtered.map(function (item) {
      ...
      var chipsHtml = '';
      if (item.unit === 'kg') {
        chipsHtml = '<div class="pos-card-chips">' +
          '<button type="button" class="pos-card-chip" data-action="quick-add" data-id="' + esc(item.id) + '" data-grams="250">+250g</button>' +
          '<button type="button" class="pos-card-chip" data-action="quick-add" data-id="' + esc(item.id) + '" data-grams="500">+500g</button>' +
          '<button type="button" class="pos-card-chip" data-action="quick-add" data-id="' + esc(item.id) + '" data-grams="1000">+1kg</button>' +
          '<button type="button" class="pos-card-chip weigh" data-action="open-modal" data-id="' + esc(item.id) + '" title="Weigh exact grams">⚖️</button>' +
        '</div>';
      } else {
        chipsHtml = '<div class="pos-card-chips">' +
          '<button type="button" class="pos-card-chip" data-action="quick-add" data-id="' + esc(item.id) + '" data-count="1">+1</button>' +
          '<button type="button" class="pos-card-chip" data-action="quick-add" data-id="' + esc(item.id) + '" data-count="2">+2</button>' +
          '<button type="button" class="pos-card-chip weigh" data-action="open-modal" data-id="' + esc(item.id) + '" title="Select quantity">Qty ▾</button>' +
        '</div>';
      }

      return '<div class="pos-item-card" data-id="' + esc(item.id) + '">' +
        '<div class="pos-item-img-wrap" data-action="open-modal" data-id="' + esc(item.id) + '">' +
          '<img src="' + imgPath + '" alt="' + esc(item.name) + '" loading="lazy" onerror="this.src=\'assets/img/kaju-katri-400.jpg\'">' +
          '<span class="pos-item-quick-weigh-badge">⚖️ Weigh</span>' +
        '</div>' +
        '<div class="pos-item-title" data-action="open-modal" data-id="' + esc(item.id) + '">' + esc(item.name) + '</div>' +
        (item.nameHi ? '<div class="pos-item-deva" data-action="open-modal" data-id="' + esc(item.id) + '">' + esc(item.nameHi) + '</div>' : '') +
    ...
    // Quick add chips listener
    itemsContainer.querySelectorAll('.pos-card-chip').forEach(function (chip) {
      chip.addEventListener('click', function (e) {
        ...
        if (action === 'quick-add') {
          if (item.unit === 'kg') {
            var grams = parseInt(this.getAttribute('data-grams'), 10) || 250;
            var lineAmount = (Number(item.rate) / 1000.0) * grams;
            addDirectToCart(item, grams, 1, lineAmount);
          } else {
            var count = parseInt(this.getAttribute('data-count'), 10) || 1;
            var lineAmount = Number(item.rate) * count;
            addDirectToCart(item, 0, count, lineAmount);
          }
        } else {
          openWeighModal(item);
        }
    ...
    itemsContainer.querySelectorAll('.pos-item-card').forEach(function (tile) {
      tile.addEventListener('click', function (e) {
        if (e.target.closest('.pos-card-chip')) return;
        var id = this.getAttribute('data-id');
        var item = catalogueItems.find(function (it) { return it.id === id; });
  ...
  // Direct Fast Add to Cart (from mobile card chips)
  function addDirectToCart(item, grams, count, lineAmount) {
    cart.push({
      id: item.id + '-' + Date.now(),
      name: item.name,
      nameHi: item.nameHi,
    ...
    var wtText = item.unit === 'kg' ? (grams >= 1000 ? (grams/1000).toFixed(1) + 'kg' : grams + 'g') : count + ' ' + (item.unit === 'box' ? 'box' : 'pc');
    showNotification('✅ Added ' + wtText + ' ' + item.name + ' (' + inr(lineAmount) + ') to Bill!');
  ...
  function generatePdfPromise() {
    ...
    // Style container inside host for clean full-width clone
    container.style.cssText = 'position: relative !important; left: 0 !important; top: 0 !important; width: 100% !important; opacity: 1 !important; display: block !important;';
  ...
  // Connect Mobile Back Buttons & Bottom Bar
  var backBtn = document.getElementById('pos-btn-back-catalogue');
  if (backBtn) {
  ...
  var browseBtn = document.getElementById('pos-btn-browse-sweets');
  if (browseBtn) {
  ...
  var mobileBar = document.getElementById('pos-mobile-bottom-bar');
  if (mobileBar) {
    mobileBar.addEventListener('click', function (e) {
      if (e.target.id === 'pos-m-bar-view-btn' || e.target.closest('#pos-m-bar-view-btn')) {
