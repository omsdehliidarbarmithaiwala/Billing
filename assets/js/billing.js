...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function showNotification(msg) {
    ...
    if (!t) { t = document.createElement('div'); t.id = 'pos-toast'; document.body.appendChild(t); }
    t.innerHTML = msg;
  ...
  var SHOP_ADDR = "Shop No. 20, Shivaji Nagar, Delisle Road, Lower Parel (E), Mumbai ÔÇö 400 013";
  ...
  function inr(n) { return 'Ôé╣' + Number(n).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
  function inrRound(n) { return 'Ôé╣' + Number(Math.round(n)).toLocaleString('en-IN'); }
  function esc(s) {
  ...
  if (typeof BOXES !== 'undefined') {
    BOXES.forEach(function (bx, i) {
      catalogueItems.push({
        ...
        nameHi: 'ÓñíÓÑìÓñ░Óñ¥Óñê Óñ½ÓÑìÓñ░ÓÑéÓñƒ Óñ¼ÓÑëÓñòÓÑìÓñ©',
        category: 'boxes',
        categories: ['boxes'],
        rate: bx.price,
        unit: 'box',
  ...
  if (typeof HAMPERS !== 'undefined') {
    HAMPERS.forEach(function (h, i) {
      catalogueItems.push({
        ...
        nameHi: h.nameHi || 'ÓñùÓñ┐Óñ½ÓÑìÓñƒ Óñ╣ÓÑêÓñéÓñ¬Óñ░',
        category: 'boxes',
        categories: ['boxes'],
        rate: h.price,
        unit: 'box',
  ...
  function recalcModalGramPrice(grams) {
    ...
    modalCalcFormula.textContent = '(' + inrRound(rate) + ' ├À 1000g) ├ù ' + grams + 'g';
  ...
  function recalcModalCountPrice(count) {
    ...
    modalCalcFormula.textContent = inrRound(rate) + ' ├ù ' + count + ' ' + unitWord;
    modalCalcPrice.textContent = inr(total);
  ...
  document.getElementById('pos-btn-custom-item').addEventListener('click', function () {
    ...
    if (!name) return;
    var rate = parseFloat(prompt('Enter Rate per Kg (or per piece) in Ôé╣:', '600')) || 600;
  ...
  function renderCart() {
    ...
    document.getElementById('pos-sum-box-charge').textContent = boxCharge > 0 ? '+' + inr(boxCharge) : 'Ôé╣0.00';
    document.getElementById('pos-sum-discount').textContent = discAmount > 0 ? '-' + inr(discAmount) : '-Ôé╣0.00';
    ...
    if (mCount) mCount.textContent = cart.length + ' items ' + (totalGrams > 0 ? '┬À ' + (totalGrams >= 1000 ? (totalGrams/1000).toFixed(3) + ' kg' : totalGrams + ' g') : '');
  ...
  function buildWhatsAppBillMessage(isBackup) {
    ...
    var itemLines = cart.map(function (row, idx) {
      ...
      var wtStr = row.unit === 'kg' ? (row.grams >= 1000 ? (row.grams/1000).toFixed(3) + ' kg' : row.grams + ' g') : row.count + ' ' + row.unit;
      if (row.unit === 'kg') totalGrams += row.grams;
      return (idx + 1) + '. *' + row.name + '*\n   ' + inrRound(row.rate) + '/' + row.unit + ' ├ù ' + wtStr + ' = *' + inr(row.amount) + '*';
    }).join('\n');
    ...
    var header = isBackup ? '­ƒøí´©Å *[SHOP RECORD & BACKUP COPY]*\n' : '';

    var msg = header +
              '­ƒææ *OM\'S DEHLII DARBAR MITHAIWALA (Since 1947)*\n' +
              '­ƒôì *Shop No. 20, Delisle Road, Lower Parel, Mumbai ÔÇö 400 013*\n' +
              '­ƒô× *Hotline:* ' + SHOP_PHONES + '\n' +
              '­ƒîÉ *Our Official Website & Menu:* ' + SHOP_WEBSITE + '\n' +
              '­ƒÅø´©Å *GSTIN:* ' + SHOP_GST + ' | *FSSAI Lic:* ' + SHOP_FSSAI + '\n' +
              '-----------------------------------------\n' +
              '­ƒº¥ *TAX INVOICE / CASH BILL*\n' +
              '­ƒöó *Bill No:* ' + invoiceNum + '\n' +
              '­ƒôà *Date:* ' + dateStr + ' ' + timeStr + '\n' +
              '­ƒæñ *Customer:* ' + custName + ' (' + (phoneInput || 'Counter Sale') + ')\n' +
              '-----------------------------------------\n' +
              itemLines + '\n' +
              '-----------------------------------------\n' +
              '­ƒÆ░ *Subtotal:* ' + inr(subtotal) + '\n';

    if (boxCharge > 0) msg += '­ƒÄü *Luxury Sweet Box:* ' + inr(boxCharge) + '\n';
    if (discAmount > 0) msg += '­ƒÅÀ´©Å *Discount (' + discPct + '%):* -' + inr(discAmount) + '\n';

    msg += 'Ô¡É *GRAND TOTAL: ' + inr(grandTotal) + '*\n' +
           '-----------------------------------------\n' +
           '­ƒôä *Official PDF Tax Invoice attached / generated.*\n' +
           '­ƒîÉ *Explore all sweets on our website:* ' + SHOP_WEBSITE + '\n' +
           'Ô£¿ *Freshness Rule:* Milk & Mawa sweets consume within 12 hours. Bengali sweets keep in fridge.\n' +
           '­ƒÖÅ *Thank you for shopping with us! Have a sweet day!*';
  ...
  function generatePdfPromise() {
    ...
    var filename = 'Invoice_' + invoiceNum + '_OmsDehliDarbar.pdf';
  ...
  // AUTOMATIC SHOP BACKUP TRIGGER (+91 9820260299)
  ...
  document.getElementById('btn-send-whatsapp').addEventListener('click', async function () {
    ...
    try {
      ...
      // Then open customer WhatsApp
      var waUrl = 'https://wa.me/' + finalPhone + '?text=' + encodeURIComponent(msg);
      window.open(waUrl, '_blank');

      // Auto-trigger shop backup to 9820260299
  ...
  // MANUAL SHOP BACKUP (btn-send-backup)
  ...
  document.getElementById('btn-send-sms').addEventListener('click', function () {
    ...
    var smsText = "Om's Dehlii Darbar Bill " + invoiceNum + " for " + custName + ": Total " + inr(grandTotal) + " (" + cart.length + " items). Visit: " + SHOP_WEBSITE + " | GSTIN: " + SHOP_GST + ". Thank you!";
